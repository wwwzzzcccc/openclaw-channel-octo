...
The agent reads the root anchor and latest revision, submits its narrow edit using revision CAS, and reads the result back. If it only delivers a planning reply and two successful revision reads show no change, the handler permits one execution continuation. Each round has a separate delivery report; a failed continuation cannot inherit the first round's delivery report. Revision reads that fail never enable a continuation.

Both PPT rounds share the resolved account dispatch budget (660 seconds by default, also used with an error log if live configuration lookup fails); the continuation receives only the time remaining on the original deadline. Expired tasks do not start another Agent turn. Timeout and account shutdown signal the underlying Agent to cancel, with a bounded cancellation grace. Account shutdown also cancels in-flight revision reads and comment posts; it does not send a fresh fallback. A host that ignores cancellation may still have an uncertain side effect, so PPT persists deduplication before Agent handoff and never automatically replays that started event. A known stop before runtime invocation rolls back only its unstarted reservation. Polling leaves a PPT event unacknowledged if the account stops while handling it. Legacy and HTML document tasks instead finish their existing cursor/ACK receipt on stop, because they do not hold PPT pre-handoff reservations; no subsequent task in the batch starts after stop. A process crash between durable reservation and handoff can leave an unexecuted reservation; inspect the document before issuing a new mention.

The planning classifier only gates the optional continuation and emits operator diagnostics. It never vetoes a delivered final answer or adds a failure notice beneath it. A delivered answer is not proof that a deck edit succeeded: the prompt still requires read-back verification, and revision growth alone cannot identify the actor or target. A continuation that fails before delivering an answer gets an uncertainty notice without automatically repeating the edit. Replies retain event deduplication and do not resolve their root thread. See [the PPT wire contract](docs/ppt-contract.md) for verified HTTP shapes and ID boundaries.

Unsupported nonempty `doc_kind` values never select a fallback API or start an Agent. The poller records `unsupported_doc_kind` in the existing per-account `doc-tasks.deadletter.json` (bounded to 200 entries), logs the event ID/kind, and advances its durable cursor without ACK. This avoids repeatedly executing later events behind an unknown-protocol gap. The server retains the original unacknowledged event until its own expiry; recovery after a consumer upgrade is an explicit operator action, not automatic replay. The diagnostics store is best effort, with write failures logged under the existing disk-failure policy. Upgrade every consuming plugin and CLI before the Server and Docs PPT producer. The only accepted nonempty wire kinds are `html` and `ppt`; legacy doc/sheet/board omit the kind. The stored `html_ppt` document type is not a wire alias.

PPT revision-read failures log only document/thread IDs and HTTP status (or `unknown`) and disable continuation. No recurring poller-status interface is added to account snapshots.
